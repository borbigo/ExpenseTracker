...
struct Transaction: Identifiable, Decodable, Hashable {
    ...
    var icon: FontAwesomeCode {
        //if a matching category is found, return that icon
        if let category = Category.all.first(where: { $0.id == categoryId }) {
            return category.icon
        }
        //otherwise, return the question icon
        return .question
    }
    ...
    var month: String {
        //.wide makes sure that we show the full name of the month
...
extension Category {
    ...
    //SUBCATEGORIES - has a mainCaegoryId to fall under the maincategories
